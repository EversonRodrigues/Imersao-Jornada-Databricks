...
Imersão Jornada de Dados: e-commerce brasileiro no Databricks Free Edition, em 4 aulas. Pipeline medalhão no catálogo `ecommerce` (schemas `raw`, `bronze`, `silver`, `gold`), dashboards AI/BI e um Genie space. Tudo é implantado como Declarative Automation Bundle (`databricks.yml`).

## Estrutura

- `aula-01-sql-dashboard/`: notebook SQL da Aula 1 (cria `ecommerce.raw.*`) e dashboard sobre `raw`.
